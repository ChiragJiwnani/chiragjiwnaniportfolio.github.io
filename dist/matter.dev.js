...
(function (w) {
  ...
  function init() {
    ...
    ctx = canvas.getContext("2d"); //These two set the width and height of the canvas to the defined values.
    ...
    for (col = 0; col < num_cols; col++) {
      ...
      vec_cells[col] = []; //This loops through the count of rows.

      for (row = 0; row < num_rows; row++) {
        var cell_data = new cell(col * resolution, row * resolution, resolution); //This pushes the cell object into the grid array.
    ...
    for (col = 0; col < num_cols; col++) {
      for (row = 0; row < num_rows; row++) {
        ...
        var col_right = col + 1 < num_cols ? col + 1 : 0; //Get the reference to the cell on the row above.
    ...
    canvas.addEventListener("touchmove", touch_move_handler); //When the page is finished loading, run the draw() function.
  ...
  function update_particle() {
    ...
    for (i = 0; i < particles.length; i++) {
      ...
      var p = particles[i]; //If the particle's X and Y coordinates are within the bounds of the canvas...
      ...
      if (p.x >= 0 && p.x < canvas_width && p.y >= 0 && p.y < canvas_height) {
        ...
        var row = parseInt(p.y / resolution); //Same as above, store reference to cell
        ...
        p.yv += ay * cell_data.down.yv * 0.05; //This adds the calculated velocity to the position coordinates of the particle.

        p.x += p.xv;
        p.y += p.yv; //For each axis, this gets the distance between the old position of the particle and it's new position.

        var dx = p.px - p.x;
        var dy = p.py - p.y; //Using the Pythagorean theorum (A^2 + B^2 = C^2), this determines the distance the particle travelled.

        var dist = Math.sqrt(dx * dx + dy * dy); //This line generates a random value between 0 and 0.5

        var limit = Math.random() * 0.5; //If the distance the particle has travelled this frame is greater than the random value...
        ...
        p.y = p.py = Math.random() * canvas_height; //Set the particles velocity to zero.
  ...
  function draw() {
    ...
    var mouse_yv = mouse.y - mouse.py; //Loops through all of the columns

    for (i = 0; i < vec_cells.length; i++) {
      var cell_datas = vec_cells[i]; //Loops through all of the rows

      for (j = 0; j < cell_datas.length; j++) {
        //References the current cell
        var cell_data = cell_datas[j]; //If the mouse button is down, updates the cell velocity using the mouse velocity
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); //This sets the color to draw with.

    ctx.strokeStyle = "#00FFFF"; //This calls the function to update the particle positions.
    ...
    mouse.px = mouse.x;
    mouse.py = mouse.y; //This requests the next animation frame which runs the draw() function again.
  ...
  function change_cell_velocity(cell_data, mvelX, mvelY, pen_size) {
    ...
    var dist = Math.sqrt(dy * dy + dx * dx); //If the distance is less than the radius...
  ...
  function update_pressure(cell_data) {
    ...
    var pressure_y = cell_data.up_left.yv * 0.5 + cell_data.up.yv + cell_data.up_right.yv * 0.5 - cell_data.down_left.yv * 0.5 - cell_data.down.yv - cell_data.down_right.yv * 0.5; //This sets the cell pressure to one-fourth the sum of both axis pressure.
  ...
  function update_velocity(cell_data) {
    /*
        ...
        */
    cell_data.xv += (cell_data.up_left.pressure * 0.5 + cell_data.left.pressure + cell_data.down_left.pressure * 0.5 - cell_data.up_right.pressure * 0.5 - cell_data.right.pressure - cell_data.down_right.pressure * 0.5) * 0.25; //This does the same for the Y axis.
  ...
  function cell(x, y, res) {
    ...
    this.x = x;
    this.y = y; //This is the width and height of the cell

    this.r = res; //These are the attributes that will hold the row and column values

    this.col = 0;
    this.row = 0; //This stores the cell's velocity

    this.xv = 0;
    this.yv = 0; //This is the pressure attribute
  ...
  function mouse_move_handler(e) {
    ...
    mouse.px = mouse.x;
    mouse.py = mouse.y; //Sets the new coordinates
  ...
  function touch_move_handler(e) {
    ...
    mouse.px = mouse.x;
    mouse.py = mouse.y; //This line gets the coordinates for where the canvas is positioned on the screen.
...
window.requestAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame; //And this line calls the init() function defined above to start the script.

Fluid.initialize();
//# sourceMappingURL=matter.dev.js.map
